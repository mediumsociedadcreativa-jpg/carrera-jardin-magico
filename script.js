...
canvas.addEventListener("click", (evento) => {
    ...
    if (estadoJuego === "SELECCION") {
        ...
        for (let c of corredores) {
            // Rango de toque adaptado al tamaño de los sprites de 256x256 px
...
// --- LÓGICA DE CONTROL DE ACCESO ÚNICO (NUEVO) ---
function verificarAccesoYArrancar() {
    ...
    if (!usuarioRegistrado) {
        ...
        // Siguientes ocasiones: Entra directo sin pedir código
...
function pedirCodigoAcceso() {
    let codigo = prompt("Por favor, introduce tu código de acceso para registrar la instalación:");
    
    // Si cancela o deja vacío, no lo dejamos pasar
    ...
    // Validación del código (Cámbialo por el tuyo)
    if (codigo === "MARIPOSA2026") { 
        alert("¡Código correcto! Registrando instalación...");
        
        // Guardamos de forma permanente en el dispositivo
        localStorage.setItem('jardin_magico_registrado', 'true');
        
        // Enviar a tu servidor/hoja de cálculo si es necesario trackear quién fue
        registrarEnServidor(codigo);

        // Arrancamos el juego con normalidad
        prepararSeleccion();
        loop();
    } else {
...
function registrarEnServidor(codigo) {
    // Si en un futuro agregas una API para saber quién lo instaló, la llamas aquí.
    // Al ser la primera vez, el usuario aún cuenta con el internet que usó para abrir el link.
    console.log(`Instalación trackeada exitosamente con el código: ${codigo}`);
}


// --- REEMPLAZO DE LA FUNCIÓN DE INICIO ORIGINAL ---
cargarImagenesJuego(() => {
    // En lugar de iniciar directo, pasa por el filtro de seguridad
